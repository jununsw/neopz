...
// Default constructor.
TDistributedLoad::TDistributedLoad(int ElementID, double Node0Load,
								   double Node1Load, bool LoadPlane,
...
void TDistributedLoad::store(TPZFMatrix<double>& L) {
	...
	if (fLoadPlane == true) {
		...
		L(fy0DOF, 0) +=
			(7 * fNode0Load * length / 20) + (3 * fNode1Load * length / 20);
		if (m0DOF != -1) {
			L(m0DOF, 0) += (fNode0Load * length * length * lx / 30) +
				(fNode1Load * length * length * lx / 20);
		}

		L(fy1DOF, 0) +=
			(3 * fNode0Load * length / 20) + (7 * fNode1Load * length / 20);
		if (m1DOF != -1) {
			L(m1DOF, 0) += -1 * ((fNode0Load * length * length * lx / 20) +
				(fNode1Load * length * length * lx / 30));
		}
	}
	...
	// If the load plane is the element local plane.
		...
		L(fx0DOF, 0) += -1 * ly * ((7 * fNode0Load * length / 20) +
			(3 * fNode1Load * length / 20));
		L(fy0DOF, 0) +=
			lx * ((7 * fNode0Load * length / 20) + (3 * fNode1Load * length / 20));
		if (m0DOF != -1) {
			L(m0DOF, 0) += (fNode0Load * length * length / 30) +
				(fNode1Load * length * length / 20);
		}

		L(fx1DOF, 0) += -1 * ly * ((3 * fNode0Load * length / 20) +
			(7 * fNode1Load * length / 20));
		L(fy1DOF, 0) +=
			lx * ((3 * fNode0Load * length / 20) + (7 * fNode1Load * length / 20));
		if (m1DOF != -1) {
			L(m1DOF, 0) += -1 * ((fNode0Load * length * length / 20) +
				(fNode1Load * length * length / 30));
		}
	}
}
